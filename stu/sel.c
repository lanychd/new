...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (Ino[0] == '*')
	{
		sprintf(sql, "select Iname,Isex,Iage,Iphone,Sdept,Cno,grade from information,score where information.Ino=score.Ino and information.fl = '1'",Ino);
	}
	else
	{
		sprintf(sql, "select Iname,Isex,Iage,Iphone,Sdept ,Cno,grade from information,score  where information.Ino=score.Ino and information.Ino='%s' and information.fl ='1'", Ino);
	}
